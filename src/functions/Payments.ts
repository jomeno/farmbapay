...
export async function Payments(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    ...
    context.log(`Environemtn variable ${process.env.CLIENT_EMAIL}`);
    ...
    const result = spreadsheet.insert("1JUyYCV9cn2icAvSPuLHP-Ju7OEJ5p5p07kAWJPs9x58", "Pay Notifications", newValues);
